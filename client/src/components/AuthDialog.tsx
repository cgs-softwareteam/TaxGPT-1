...
export function AuthDialog({
  ...
  // Local state for the magic-link sign-in flow.
  // Three views: 'idle' shows OAuth + "Continue with email" buttons,
  // 'input' shows the email field, 'sent' confirms the link was sent.
  const [magicView, setMagicView] = useState<"idle" | "input" | "sent">("idle");
    ...
    >
      ...
      >
        ...
        <div className="space-y-3 pt-2">
          {magicView === "idle" && (
            <>
              ...
              <Button
                ...
                onClick={() => {
                  setMagicView("input");
                  trackEvent("magic_link_started", { mode });
                }}
                className="w-full"
                data-testid="auth-dialog-button-email-continue"
              >
          ...
          {magicView === "input" && (
            <form onSubmit={submitMagicLink} className="space-y-3" data-testid="magic-link-input-form">
          ...
          {magicView === "sent" && (
            <div className="space-y-3" data-testid="magic-link-sent">
              <div className="flex items-start gap-2 text-sm text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-950/30 p-4 rounded-md">
                <CheckCircle2 className="h-5 w-5 shrink-0 mt-0.5" />
                <div>
                  <p className="font-medium mb-1">Check your inbox</p>
                  <p>
                    We sent a sign-in link to{" "}
                    <span className="font-mono">{magicEmail}</span>. The link
              ...
              <button
                ...
                onClick={resetMagic}
                className="text-xs text-gray-500 hover:text-gray-700 hover:underline w-full text-center"
                data-testid="magic-link-sent-back"
              >
        ...
        {/* Email-capture alternative — only shown when there's an actual
            report to send AND we're not in the middle of the magic-link
            flow (avoids two email inputs being on screen at once). */}
        {latestReport && magicView === "idle" && (
